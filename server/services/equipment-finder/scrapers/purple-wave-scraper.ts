...
/**
 * Purple Wave — online agricultural & industrial auctions.
 * Search: https://www.purplewave.com/auction?q=<query>
 */
export class PurpleWaveScraper extends HtmlEquipmentScraper {
  readonly source: ListingSource = 'purple_wave';
  readonly name = 'Purple Wave';

  private readonly base = 'https://www.purplewave.com';

  async search(config: ScraperConfig): Promise<ScraperResult> {
    const start = Date.now();
    const url = `${this.base}/auction?q=${encodeURIComponent(config.query)}`;
    ...
    $('a[href*="/item/"], a[href*="/lot/"]').each((_, el) => {
      ...
      const sourceId = href.replace(/^.*\/(?:item|lot)\//, '').replace(/\/.*/, '') || href;
      ...
      listings.push({
        ...
        seller: { isDealer: true, name: 'Purple Wave' },
        imageUrls: [],
        imageCount: 0,
