...
/**
 * Ritchie Bros Auctioneers — the largest heavy-equipment auction house.
 * Search: https://www.rbauction.com/search?keywords=<q>
 * NOTE: selectors may need live-tuning; rbauction.com ships a React SPA and
 * this scraper targets the server-rendered HTML surface.
 */
export class RitchieBrosScraper extends HtmlEquipmentScraper {
  readonly source: ListingSource = 'ritchie_bros';
  readonly name = 'Ritchie Bros Auctioneers';

  private readonly base = 'https://www.rbauction.com';
  ...
  async search(config: ScraperConfig): Promise<ScraperResult> {
    ...
    $('a[href*="/lot/"], a[href*="/equipment/"]').each((_, el) => {
      ...
      const currentBid = parseMoney(text.match(/(?:Current|Latest)\s+bid[^\d]*\$?([\d,]+)/i)?.[1]);
      const state = detectState(text);
      const endMatch = text.match(/(Ends?|Closes?)\s+([A-Za-z0-9:,\-/ ]{6,50})/i);
      const sourceId = href.replace(/^.*\/(?:lot|equipment)\//, '').replace(/\/.*/, '') || href;
      ...
      listings.push({
        ...
        seller: { isDealer: true, name: 'Ritchie Bros' },
        imageUrls: [],
        imageCount: 0,
