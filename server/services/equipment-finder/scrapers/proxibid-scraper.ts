...
/**
 * Proxibid — general industrial & equipment auctions.
 * Search: https://www.proxibid.com/search?search-query=<query>
 */
export class ProxibidScraper extends HtmlEquipmentScraper {
  readonly source: ListingSource = 'proxibid';
  readonly name = 'Proxibid';

  private readonly base = 'https://www.proxibid.com';

  async search(config: ScraperConfig): Promise<ScraperResult> {
    const start = Date.now();
    const url = `${this.base}/search?search-query=${encodeURIComponent(config.query)}`;
    ...
    $('a[href*="/lot/"], a[href*="/LotDetail"]').each((_, el) => {
      ...
      const sourceId = href.replace(/^.*\/(?:lot|LotDetail)[/?=]/, '').replace(/\/.*/, '') || href;
      ...
      listings.push({
        ...
        seller: { isDealer: true, name: 'Proxibid' },
        imageUrls: [],
        imageCount: 0,
