...
/**
 * EquipmentTrader — dealer network for commercial / work equipment.
 * Search: https://www.equipmenttrader.com/find?keyword=<query>
 */
export class EquipmentTraderScraper extends HtmlEquipmentScraper {
  readonly source: ListingSource = 'equipment_trader';
  readonly name = 'EquipmentTrader';

  private readonly base = 'https://www.equipmenttrader.com';

  async search(config: ScraperConfig): Promise<ScraperResult> {
    const start = Date.now();
    const url = `${this.base}/find?keyword=${encodeURIComponent(config.query)}`;
    ...
    $('a[href*="/listing/"], a[href*="/equipment/"]').each((_, el) => {
      ...
      const sourceId = href.replace(/^.*\/(?:listing|equipment)\//, '').replace(/\/.*/, '') || href;
